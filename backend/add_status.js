...
async function run() {
  ...
  try {
    ...
    await pool.query("ALTER TABLE item_variants ADD COLUMN status VARCHAR(50) DEFAULT 'ACTIVE'");
    console.log("Success adding status column.");
  } catch (err) {
  ...
  try {
    console.log("Updating v_variants_full view...");
    await pool.query(`
