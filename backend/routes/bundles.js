...
// GET /api/bundles/:id - Detail bundle dengan semua variant
...
// POST /api/bundles - Buat Bundle (Product + GoTo Ecommerce)
router.post('/', authenticate, requireRole(['product', 'goto_ecommerce']), async (req, res) => {
  ...
  try {
    ...
    // Validasi semua variant_id exist
    for (const v of variants) {
      ...
      if (!vExist) return res.status(404).json({ success: false, message: `Variant ID ${v.variant_id} tidak ditemukan.` });
    }
    ...
    const bundle_id = bundleResult.insertId;
    ...
    res.status(201).json({ success: true, message: `Bundle berhasil dibuat dengan SKU: ${bundle_sku}`, data: { ...newBundle, details } });
  } catch (err) {
    await conn.rollback();
    console.error(err);
...
// PUT /api/bundles/:id - Update Bundle header (Product only)
...
// DELETE /api/bundles/:id (Product only)
