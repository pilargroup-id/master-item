...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    // For free text categories, we could count distinct category names
    const [[{ total_categories }]] = await pool.query('SELECT COUNT(DISTINCT category_name) AS total_categories FROM item_parents WHERE category_name IS NOT NULL AND category_name != ""');
    ...
    const [recent_variants] = await pool.query(`
      ...
      FROM item_variants iv JOIN item_parents ip ON iv.parent_id = ip.id
    ...
    res.json({
      ...
      data: {
        ...
        seq_pools: {
          parent_last_seq: seqParent.last_seq || 0,
          variant_bundle_last_seq: seqVB.last_seq || 0
        },
