...
async function run() {
  try {
    ...
    const createViewSql = `
      ...
      LEFT JOIN sku_pic_categories spc ON sc.name = spc.detail_category
      LEFT JOIN sku_pics sp ON spc.pic_id = sp.id
    `;
    
    await pool.query(createViewSql);
    console.log("View v_variants_full recreated successfully with PIC");
  } catch (err) {
