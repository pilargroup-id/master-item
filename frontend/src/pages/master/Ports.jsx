...
import { Plus, Search, Edit, Trash2, Database, X, Save, Anchor } from 'lucide-react';
...
function PortModal({ item, onClose, onSave }) {
  ...
  useEffect(() => {
    if (item) {
      setFormData({ port_name: item.port_name || '' });
    } else {
      setFormData({ port_name: '' });
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await axios.put(`/api/master/ports/${item.id}`, formData);
      } else {
        await axios.post('/api/master/ports', formData);
      }
      onSave();
    } catch (err) {
      ...
      <div className="modal-box" style={{ width: '100%', maxWidth: '450px' }}>
        <div className="modal-header">
          <div className="d-flex align-items-center gap-3">
            ...
            <div>
              <div className="modal-header-title">{isEdit ? 'Edit Port' : 'Tambah Port'}</div>
              <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginTop: '0.1rem' }}>
                {isEdit ? `Mengedit: ${item.port_name}` : 'Tambah data master Port'}
              </div>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div className="form-group" style={{ marginBottom: 0 }}>
            <label>Port Name <span style={{ color: 'var(--danger)' }}>*</span></label>
            <input type="text" required value={formData.port_name} onChange={set('port_name')} placeholder="Contoh: TANJUNG PRIOK" />
          </div>
...
export default function Ports() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await axios.get('/api/master/ports');
  ...
  const handleDelete = async (id, name) => {
    if (!window.confirm(`Hapus Port "${name}"?`)) return;
    try {
      const res = await axios.delete(`/api/master/ports/${id}`);
  ...
  const filtered = data.filter(d => (d.port_name || '').toLowerCase().includes(search.toLowerCase()));
  const totalPages = Math.max(1, Math.ceil(filtered.length / pageSize));
  const paginated = filtered.slice((page - 1) * pageSize, page * pageSize);

  return (
    <div className="animate-fade-in">
      {showModal && <PortModal item={selectedItem} onClose={closeModal} onSave={afterSave} />}
      <div className="page-header">
        <div className="page-header-left">
          <h1>Master Ports</h1>
          <p>Kelola data master Pelabuhan (Port)</p>
        </div>
        {isProductDivision() && (
      ...
      <div className="card" style={{ padding: '1.25rem' }}>
        <div className="d-flex gap-3 align-items-center mb-3" style={{ flexWrap: 'wrap' }}>
          <div className="search-wrapper">
            ...
            <input type="text" placeholder="Cari port..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
              ...
              <tbody>
                {paginated.map((item, idx) => (
                  <tr key={item.id}>
                    ...
                    <td style={{ fontWeight: 600 }}>{item.port_name}</td>
                    {isProductDivision() && (
                      <td>
                        <div className="d-flex gap-2" style={{ justifyContent: 'center' }}>
                          ...
                          <button onClick={() => handleDelete(item.id, item.port_name)} className="btn-icon" title="Hapus" style={{ color: 'var(--danger)', background: 'var(--danger-light)', border: '1.5px solid var(--danger-border)' }}><Trash2 size={14} /></button>
                        </div>
                      </td>
                    )}
                  </tr>
